...
//            out.println("</html>");
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            Control ask = new Control(ID);
            String rutaCon = "http://localhost:8084/SASME";
            if(ask.PERMISO(PASS)){
                ask.GenTicket(request.getRemoteAddr(),"1");
                String token = ask.GetTicket();
                response.sendRedirect(rutaCon+"/AceptE.jsp?ID="+ID+"&token="+token.replace("+", "/plus/"));
            }else{
